...
function renderPlans(plans) {
  $("plansGrid").innerHTML = (plans || []).map(plan => `
    <div class="plan-card ${selectedPlanId === plan.id ? "active" : ""}">
      ...
      <div class="price">${plan.price_inr ? `₹${plan.price_inr}` : "Free"}</div>
...
function updateAuthBanner(auth) {
  ...
  if (auth?.authenticated && auth.user) {
    ...
    $("authStateText").textContent = `${auth.user.email} • ${auth.user.role} • ${auth.user.plan?.name || auth.user.plan_id}`;
...
async function loadAuthState() {
  ...
  if (data?.authenticated && data.user?.email === "piyushmani33@gmail.com") {
    $("masterEmail").value = data.user.email;
  }
}
...
function continueSession() {
  const path = nextPath();
  const destination = authMe?.user?.role === "master" ? path : "/agent";
