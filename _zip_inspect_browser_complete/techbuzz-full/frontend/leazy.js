...
function renderMotherMonitor(data) {
  ...
  if ($("monitorStatus")) {
    $("monitorStatus").textContent = `Heartbeat ${data.heartbeat_ms || 0} ms with ${domains.length} domains, ${telemetry.organs_online || 0} organs, and ${telemetry.cells_active || 0} active cells`;
  }
  if ($("monitorSummary")) {
    $("monitorSummary").textContent = `Last scan ${data.last_scan_at || "unknown"}. Mother brain is tracking ${data.engines?.length || 0} engines, ${organSystems.length || 0} organ systems, and ${nervous.transmissions?.length || 0} live transmissions.`;
  }
  if ($("systemsStatus")) {
    $("systemsStatus").textContent = `${domains.length} operational domains and ${organSystems.length || 0} organ systems are connected`;
  }
  if ($("systemsSummary")) {
    const leaders = domains.slice(0, 3).map(domain => domain.lead).join(", ");
    $("systemsSummary").textContent = leaders ? `Lead agents: ${leaders}. Signal integrity ${telemetry.signal_integrity || 0}%.` : "Operational leaders will appear here.";
  }
  if ($("monitorStrip")) {
    const strip = [
  ...
  if ($("monitorAlerts")) {
    $("monitorAlerts").innerHTML = alerts.length ? alerts.map(item => `
      <div class="proposal-card">
        ...
        <p>${item.summary}</p>
        <div class="proposal-meta">${item.level || "info"}</div>
      </div>
  ...
  if ($("monitorReports")) {
    $("monitorReports").innerHTML = reports.length ? reports.map(item => `
      <div class="proposal-card">
        <strong>${item.engine || item.name || "Engine Report"}</strong>
        <p>${item.summary || "No summary yet."}</p>
        <div class="proposal-meta">${item.status || "live"} - ${item.created_at || data.last_scan_at || "now"}</div>
      </div>
  ...
  if ($("nervousFlow")) {
    ...
    $("nervousFlow").innerHTML = flowRows.length ? flowRows.map(item => `
      <div class="proposal-card">
        ...
        <p>${item.message || item.label || "Flow registered."}</p>
        <div class="proposal-meta">${item.created_at || data.last_scan_at || "live"}</div>
      </div>
  ...
  if ($("organSystems")) {
    $("organSystems").innerHTML = organSystems.length ? organSystems.map(item => `
      <div class="proposal-card">
        <strong>${item.name}</strong>
        <p>${item.role}</p>
        <div class="proposal-meta">${item.status} - load ${item.load || 0}%</div>
      </div>
  ...
  if ($("cellClusters")) {
    $("cellClusters").innerHTML = cellClusters.length ? cellClusters.map(item => `
      <div class="proposal-card">
        <strong>${item.name}</strong>
        <p>${item.signal}</p>
        <div class="proposal-meta">${item.identity} - ${item.status} - ${item.cell_count || 0} cells</div>
      </div>
    ...
    $("systemsGrid").innerHTML = domains.length ? domains.map(domain => `
      <div class="system-card">
        ...
        <div class="system-meta">
          ...
          <span>${domain.priority || 0}%</span>
        </div>
        <p>${domain.purpose}</p>
        <p>${domain.latest_signal}</p>
        <div class="proposal-meta">${domain.metric || ""}</div>
      </div>
...
function renderPranaNadi(data) {
  ...
  if ($("pranaMetrics")) {
    const metrics = [
      ...
      ["Heartbeat", `${data.heartbeat_ms || 0} ms`],
      ["Hridaya Glow", `${data.hridaya?.glow || 0}%`],
  ...
  if ($("pranaMessage")) {
    $("pranaMessage").textContent =
      ...
      `${data.hridaya?.name || "Ishani-Hridaya"}: ${data.hridaya?.message || "Your will becomes the empire's heartbeat."}\n` +
  ...
  if ($("hridayaStatus")) {
    $("hridayaStatus").textContent = `${data.hridaya?.name || "Ishani-Hridaya"} • ${data.hridaya?.sync || 0}% sync`;
  }
  if ($("hridayaCopy")) {
    $("hridayaCopy").textContent = `${data.hridaya?.message || "Where your will becomes the empire's heartbeat."} ${data.hridaya?.mantra || ""}`;
  }
  const dock = $("hridayaDock");
  if (dock) {
...
async function loadVoiceStatus() {
  ...
  if ($("voiceEngineNote")) {
    $("voiceEngineNote").textContent = `Engine ${voicePrefs.engine}. Language ${voicePrefs.language}. Rate ${voicePrefs.rate.toFixed(2)}. Pitch ${voicePrefs.pitch.toFixed(2)}.`;
  }
  ...
  $("listeningState").textContent = listening ? "Listening is active. Browser speech recognition will restart when it ends." : "Listening is disabled until you enable it.";
}
...
function renderSettings(settings) {
  ...
  const providerRows = ["openai", "gemini", "anthropic", "built_in"].map(key => {
  ...
  $("settingsSummaryText").textContent = `Voice ${boolLabel(settings.voice?.always_listening)} (${voicePreferenceLabel(settings.voice?.voice_profile)}), screen vision ${boolLabel(settings.privacy?.screen_capture_enabled)}, packages ${boolLabel(settings.privacy?.bounded_packages_enabled)}.`;
}
...
async function fetchProviderModels() {
  try {
    const provider = $("providerSelect").value;
    if (provider === "auto" || provider === "built_in") {
      toast("Choose OpenAI, Gemini, or Anthropic to fetch live models.");
      return;
    }
...
async function removeSavedProviderKey() {
  try {
    const provider = $("providerSelect").value;
    if (provider === "auto" || provider === "built_in") {
      toast("Select OpenAI, Gemini, or Anthropic first.");
      return;
    }
    const data = await api("/api/providers/configure", {
      method: "POST",
    ...
    providerDraft = { provider: "built_in", model: null, dirty: false };
    clearProviderKeyInput();
    latestSettings = data.settings || latestSettings;
    renderSettings(latestSettings);
...
async function saveProviderConfig() {
  try {
    ...
    const model = $("providerModelSelect").value;
    const apiKey = $("providerKeyInput").value.trim();
    if (!apiKey && !latestSettings?.providers?.[provider]?.configured) {
...
function initRecognition() {
  ...
  recognition.onerror = event => {
    if ($("voiceStatus")) {
      $("voiceStatus").textContent = `Voice recognition issue: ${event.error || "unknown error"}\nSwitch to manual voice commands or refresh the voice relay.`;
    }
  };
  recognition.onend = () => {
...
async function testWake() {
  await submitVoiceCommand("Hey Jinn, channel Krishna and report empire status");
}
...
async function sendVoiceCommand() {
  ...
  input.value = "";
  await submitVoiceCommand(command);
}
